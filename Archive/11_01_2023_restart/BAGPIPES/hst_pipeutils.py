...
def plot_example(blob_id, input_catalog, ax=None, plot_distributions=True, spec_color='#758BFD', phot_color='#FF8600',
                 config='parametric', show_xlabel=True, show_ylabel=True):

    if ax is None:
        ax = plt.gca()

    original_id = input_catalog['fit_id']
    input_catalog['fit_id'] = [input_catalog['fit_id'][i] + '_' + config for i in range(len(input_catalog))]
    ...
    input_catalog['fit_id'] = original_id
    ...
    if plot_distributions:
        ...
        sns.kdeplot(1e3 * blob.posterior.samples['mass_weighted_age'], ax=inset_age, color=spec_color, fill=True, alpha=0.5)
        ...
        inset_age.set_xlabel(r'$\langle\;t_\star\;\rangle\mathrm{[Myr]}$', fontsize=13)
