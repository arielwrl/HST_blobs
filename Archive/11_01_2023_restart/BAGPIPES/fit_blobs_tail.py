...
parallel. Then in the end of the run I have to delete all remaining incomplete runs using
...
if run_flag is False:
    ...
    output_catalog['photometry_syn_iqr'] = np.zeros((len(blob_catalog), 5))
    ...
    if config in ['parametric', 'exp', 'exp_smalltau', 'exp_logprior', 'dexp_logprior']:
        ...
        output_catalog['tau_std'] = np.zeros(len(blob_catalog))
        output_catalog['tau_iqr'] = np.zeros(len(blob_catalog))
        output_catalog['eta'] = np.zeros(len(blob_catalog))
        output_catalog['eta_std'] = np.zeros(len(blob_catalog))
        output_catalog['eta_iqr'] = np.zeros(len(blob_catalog))
    if config == 'constant':
...
for blob_id in blob_catalog['fit_id']:
    ...
    if run_flag:
        ...
        np.savetxt('/home/ariel/Workspace/GASP/HST/Data/full_spectra/'+blob_id+'.dat',
        ...
        if config in ['parametric', 'dexp_logprior']:
            ...
            output_catalog['tau_std'][blob_index] = np.std(fit.posterior.samples['delayed:tau'])
            output_catalog['tau_iqr'][blob_index] = np.percentile(fit.posterior.samples['delayed:tau'], 75) \
            ...
            output_catalog['tau_std'][blob_index] = np.std(fit.posterior.samples['exponential:tau'])
            output_catalog['tau_iqr'][blob_index] = np.percentile(fit.posterior.samples['exponential:tau'], 75) \
                                                    - np.percentile(fit.posterior.samples['exponential:tau'], 25)
        ...
        output_catalog['photometry_syn_iqr'][blob_index] = np.percentile(fit.posterior.samples['photometry'], 75, axis=0) - np.percentile(fit.posterior.samples['photometry'], 25, axis=0)
