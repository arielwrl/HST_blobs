...
def plot_example(clump_id, input_catalog, ax=None, plot_distributions=True, spec_color='#FF8600', phot_color='#FF8600',
    ...
    photometry[:, 2] *= (1 + z) / 1e-18

    wl = clump.posterior.model_galaxy.wavelengths

    phot_25 = np.percentile(clump.posterior.samples['photometry'] * (1 + z), 16, axis=0)/1e-18
    phot_75 = np.percentile(clump.posterior.samples['photometry'] * (1 + z), 84, axis=0)/1e-18

    flux_med = np.median(clump.posterior.samples['spectrum_full'], axis=0)/1e-18
    flux_25 = np.percentile(clump.posterior.samples['spectrum_full'], 16, axis=0)/1e-18
    flux_75 = np.percentile(clump.posterior.samples['spectrum_full'], 84, axis=0)/1e-18

    ax.plot(wl, flux_med, color=spec_color, lw=0.4)
    ax.fill_between(wl, flux_25, flux_75, alpha=0.3, color=spec_color)
    ax.set_xlim(2450, 8550)
    ax.set_ylim(0, 1.35 * photometry[:, 1][0])

    ax.bar(x=photometry[:, 0], height=phot_75 - phot_25, bottom=phot_25, width=150, zorder=10, color=phot_color,
           **{'alpha': 0.5, 'linewidth': 1})
    ax.bar(x=photometry[:, 0], height=phot_75 - phot_25, bottom=phot_25, width=150, zorder=10, fill=False,
           **{'edgecolor': 'k', 'linewidth': 1.25})
    ax.errorbar(photometry[:, 0], photometry[:, 1], 1.35*photometry[:, 2], barsabove=True, fmt='ok', elinewidth=1.5,
    ...
    if plot_distributions:
        ...
        # inset_Z = ax.inset_axes([0.75, 0.28, 0.115, 0.29])
        # inset_tau = ax.inset_axes([0.88, 0.28, 0.115, 0.29])

        # inset_cont = ax.inset_axes([0.52, 0.68, 0.135, 0.29])

        sns.despine(ax=inset_age, left=True)
        sns.despine(ax=inset_mass, left=True)
        # sns.despine(ax=inset_Z, left=True)
        # sns.despine(ax=inset_tau, left=True)

        inset_age.set_yticks([])
        inset_mass.set_yticks([])
        # inset_Z.set_yticks([])
        # inset_tau.set_yticks([])

        sns.kdeplot(1e3 * clump.posterior.samples['mass_weighted_age'], ax=inset_age, color=spec_color, fill=True, alpha=0.5)
        sns.kdeplot(clump.posterior.samples['stellar_mass'], ax=inset_mass, color=spec_color, fill=True, alpha=0.5)
        # sns.kdeplot(1e3 * clump.posterior.samples['delayed:tau'], ax=inset_tau, color=spec_color, fill=True, alpha=0.5)
        # sns.kdeplot(clump.posterior.samples['delayed:metallicity'], ax=inset_Z, color=spec_color, fill=True, alpha=0.5)
        # sns.kdeplot(1e3 * clump.posterior.samples['delayed:age'], clump.posterior.samples['delayed:metallicity'], ax=inset_cont,
        #             color=spec_color, linewidth=1, fill=True)

        inset_mass.set_ylabel(r'')
        inset_age.set_xlabel(r'$\langle\;t_\star\;\rangle_M\mathrm{[Myr]}$', fontsize=13)
        inset_mass.set_xlabel(r'$\log\,M/M_\odot$', fontsize=13)
        # inset_cont.set_xlabel(r'$t_\star\;\mathrm{[Myr]}$', fontsize=13)
        # inset_cont.set_ylabel(r'$Z_\star/Z_\odot$', fontsize=13)
        # inset_Z.set_xlabel(r'$Z_\star/Z_\odot$', fontsize=13)
        # inset_tau.set_xlabel(r'$\tau\;\mathrm{[Myr]}$', fontsize=13)
